...
document.addEventListener("DOMContentLoaded", function (_e) {
  ...
  document.getElementById("bcStart").addEventListener(
    "touchstart",
    function (e) {
  ...
  document.getElementById("bcStart").addEventListener(
    ...
    function (e) {
      ...
      if (
        ...
        if (
          e.changedTouches.item(0).clientY - touchStart.y >
          window.innerHeight / 2
        ) {
   ...
   */
  document.querySelector("aside > header").addEventListener(
    "touchstart",
    function (e) {
      touchStart.x = e.changedTouches.item(0).clientX;
      touchStart.y = e.changedTouches.item(0).clientY;
  ...
  document.querySelector("aside > header").addEventListener(
    ...
    function (e) {
      if (
        ...
        var index =
          1 * document.querySelector('aside input[type="radio"]:checked').value;
        if (index < max - 1) {
   ...
   */
  document
    .querySelector('aside input[type="date"]')
    .addEventListener("change", refreshHoraires);
  /**
   *  Changement dans l'heure --> reconsultation des horaires
   */
  document
    .querySelector('aside input[type="number"]')
    .addEventListener("change", refreshHoraires);
  ...
  function refreshHoraires() {
    ...
    var date = document
      .querySelector('main aside header input[type="date"]')
      .value.split("-")
      .join("");
    var heure = document.querySelector(
      'main aside header input[type="number"]'
    ).value;
    heure = String(heure).padStart(2, "0");
    afficherHoraires(
      aside.dataset.ligne,
      aside.dataset.direction,
      aside.dataset.station,
      date,
      heure
    );
  }
  ...
  var favoris = {
    ...
    refresh: function () {
      ...
      if (tab.length == 0) {
        document.querySelector("#bcHome").innerHTML =
          "<p>Pas de station favorite.</p>";
        return;
      }
      var html = "";
      var last = null;
      for (var line of tab) {
        ...
        html +=
          ...
          "<span class='noLigne' style='color: #" +
          fg +
          "; background-color: #" +
          bg +
          ";'>" +
          lignes[l].numero +
          "</span> " +
          stations[s].lignes[l][d].split("par")[0] +
          "</li>";
      }
      html += "</ul>";
      document.querySelector("#bcHome").innerHTML =
    ...
    for (var l in lignes) {
      ...
      for (var d in lignes[l].direction) {
        r +=
          ...
          d +
          "'>" +
          "<span class='noLigne' style='background-color: #" +
          bg +
          "; color: #" +
          fg +
          ";'>" +
      ...
      fFilter = function (st) {
        return (
          st.nom
            ...
            .indexOf(
              document
                .querySelector('#bcStations input[type="text"]')
                .value.toLowerCase()
            ) >= 0
        );
      };
  ...
  function remplirStations() {
    ...
    var keys = fSort
      ? Object.keys(stations).sort(fSort)
      ...
      for (var l in station.lignes) {
        for (var ll in station.lignes[l]) {
          ...
          r +=
            ...
            lignes[l].numero +
            "</span> " +
            station.lignes[l][ll].split("par")[0] +
            "</li>";
        }
      }
      r += "</ul>";
  ...
  async function afficherHoraires(ligne, direction, station, date, heure) {
    ...
    var nom =
      (lignes[ligne].numero ? lignes[ligne].numero + " - " : "") +
      lignes[ligne].nom;
    ...
    date = String(date);
    document.querySelector('aside header input[type="date"]').value =
      date.substr(0, 4) + "-" + date.substr(4, 2) + "-" + date.substr(6);

    // récupère les trajets et les horaires
    fetch(
      URL + "/horaires/" + ligne + "/" + direction + "/" + date + "/" + heure
    ).then(
      async function (response) {
        ...
        // retrait des
        var divRadio = document.querySelector(
        ...
        if (nb == 0) {
          ...
          for (var t in horaires[key0]) {
            html +=
              "<tr" +
              (station == horaires[key0][t].arret ? " class='selected'" : "") +
              " data-station='" +
              horaires[key0][t].arret +
              "'>" +
              "<td>" +
              stations[horaires[key0][t].arret].nom +
              "</td>";
            var tab = [];
            for (var h of keys) {
